...
/**
 * Implementation of {@link ISMMConceptProvider} using NO caching. See
 * {@link SMMConceptProviderGRLCWithCache} for a caching implementation.
 *
...
public class SMMConceptProviderGRLCRemote implements ISMMConceptProvider
{
  ...
  {
    return remoteQueryAllMappedValues (sLogPrefix, sSourceNamespace, sDestNamespace);
  }
                                              ...
                                              @Nonnull final Consumer <T> aResultHandler) throws IOException
  {
    ...
    try (final HttpClientManager aMgr = new HttpClientManager (aHCFactory))
    {
      ...
      aGet.addHeader (CHttpHeader.ACCEPT, "application/json,*;q=0");
                                                            ...
                                                            @Nonnull final String sDestNamespace) throws IOException
  {
    ...
    _httpClientGetJson (aDestinationURL,
                        aJsonHandler,
                        aJson -> {
                          // Interpret result
                          if (aJson.isObject ())
                          {
                            ...
                            if (aResults != null)
                            {
                              ...
                              if (aBindings != null)
                                ...
                                {
                                  // subject (contains source namespace which
                                  // needs to be cut)
                                  String sSourceValue = aBinding.getAsObject ()
                                                                .getAsObject ("source")
                                                                .getAsString ("value");
                                  sSourceValue = StringHelper.trimStart (sSourceValue, sSourceNamespace + "#");

                                  // object (contains destination namespace
                                  // which needs to be cut)
                                  String sDestValue = aBinding.getAsObject ()
                                                              .getAsObject ("target")
                                                              .getAsString ("value");
